...
INITIAL_BUFFER_SIZE = 32 # This is the initial buffer size for the first packet. After processing the first packet's header, the client will know the expected packet size for the rest of the file (which was randomly generated by the server) and will adjust the buffer size accordingly.
packets_sent = 0
bytes_sent = 0
...
def request_all_files(connection):
    while True:
        ...
        file_received = process_packet(connection, files_dict)
        while file_received:
            file_received = process_packet(connection, files_dict)
...
'''
def process_packet(connection, files_dict):
    ...
    frame = connection.recv(INITIAL_BUFFER_SIZE).decode('utf-8')
